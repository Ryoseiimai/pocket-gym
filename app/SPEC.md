...
- 場所・器具・経験に合わない種目は候補から除外する。「はじめて」は難易度2まで、「少しある」は3までのうち衝撃系（低難易度のジャンプを含む）・片脚スクワットを除外、「慣れている」は3まで。
- 先頭に準備運動75秒（足踏み45秒＋肩回し30秒）、末尾に静的ストレッチ30秒×2を固定追加。専用ID・phase（warmup/main/cooldown）を使い、準備・整理運動は1セット・休憩0秒で保存レベルによらず一定。時間設定の種目数はメインのみ。
- 目的は部位ローテーション内の優先順に反映：体力づくりは秒数系・全身、筋力アップは回数系（セット+1、最大4）、姿勢改善は体幹・背中・引く動作、引き締めは従来のバランス。優先度が同じなら日付シード順を維持する。

### 3. トレーニング実行画面
1種目ずつ大きく表示。フォームのポイント3行。セット完了ボタン。休憩タイマー（カウントダウン表示、音なし）。375px幅で操作完結。
